...
(function() {
    ...
    const addStatusBtn = $('addStatusBtn');
    ...
    function renderStatuses() {
        ...
        let html = '';
        statuses.slice().reverse().forEach(status => {
    ...
    function unlockPremium() {
        ...
        if (code === '0000') {
            ...
            // Enable add status button
            document.getElementById('addStatusBtn')?.removeEventListener('click', openStatusModal);
            document.getElementById('addStatusBtn')?.addEventListener('click', openStatusModal);
        } else {
            premiumError.textContent = '❌ Incorrect code. Try again.';
        }
    }
    ...
    async function sendChatMessage() {
        ...
        userMsg.innerHTML = `<div class="msg-label">You</div><div class="msg-content">${escapeHtml(text)}</div><div class="msg-time">Just now</div>`;
        ...
        typing.innerHTML = `<div class="msg-label">🤖 Safari AI</div><div class="msg-content"><span class="typing-dots"><span></span><span></span><span></span></span></div>`;
        ...
        try {
            ...
            if (!reply) {
                ...
                if (lower.includes('who is your owner') || lower.includes('who created you')) {
                    reply = `🦁 I was created by <strong>${OWNER.name}</strong> from Safari Technology. Join our channel: <a href="${OWNER.channel}" target="_blank">${OWNER.channel}</a>`;
                } else if (lower.includes('channel')) {
                    reply = `📢 Join our WhatsApp channel: <a href="${OWNER.channel}" target="_blank">${OWNER.channel}</a>`;
                } else if (lower.includes('joke')) {
                    const jokes = ['Why do programmers prefer dark mode? Light attracts bugs! 🐛', 'What do you call a fake noodle? An impasta! 🍝', 'Why did the AI break up with the human? Too many bugs in the relationship! 🤖'];
            ...
            aiMsg.innerHTML = `<div class="msg-label">🤖 Safari AI</div><div class="msg-content">${parseMarkdown(reply)}</div><div class="msg-time">Just now</div>`;
            ...
            errMsg.innerHTML = `<div class="msg-label">🤖 Safari AI</div><div class="msg-content">⚠️ Error: ${escapeHtml(e.message)}</div><div class="msg-time">Just now</div>`;
    ...
    async function getAIResponse(text) {
        const persona = 'Safari AI';
        const context = `You are Safari AI, a helpful assistant created by ICEBACK MASTER TECH. Answer concisely.`;
        try {
    ...
    async function performSearch() {
        ...
        if (!query) return;
        searchResult.innerHTML = '<div class="spinner"></div> Searching...';
        try {
            const resp = await fetchWithTimeout(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(query)}`);
            if (resp.ok) {
                const data = await resp.json();
                if (data.extract) {
                    searchResult.innerHTML = `<strong>${data.title}</strong><br>${data.extract}<br><br><a href="${data.content_urls?.desktop?.page}" target="_blank" style="color:var(--accent);">Read more</a>`;
                } else {
    ...
    function researchTopic(topic) {
        researchResult.innerHTML = `<div class="spinner"></div> Researching ${topic}...`;
        setTimeout(() => {
            const facts = {
            ...
            researchResult.innerHTML = `<strong>${topic}</strong><br>${facts[topic] || 'Research data will appear here.'}`;
        }, 800);
    }
    ...
    function init() {
        ...
        // Status Modal
        addStatusBtn?.addEventListener('click', openStatusModal);
        ...
        submitStatusBtn.addEventListener('click', function() {
            const dataUrl = statusPreview.dataset.dataUrl;
            const type = statusPreview.dataset.type;
