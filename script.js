// ================================================================
// SAFARI AI – COMPLETE PLATFORM SCRIPT (FIXED & UPDATED)
// All features + Safari Search + Weather + Owner Info
// ================================================================
...
(function() {
    ...
    function parseMarkdown(text) {
        ...
        // OWNER INFO DETECTION (for direct replies)
    ...
    // ===== COMMAND HANDLERS (DIRECT, NO API NEEDED) =====
    async function handleDirectCommand(text) {
        ...
        // 3. WEATHER (weather London, weather New York)
        ...
        if (weatherMatch) {
            ...
            try {
                ...
                return {
                    reply:
                        `🌤️ <strong>Weather in ${city}</strong><br />🌡️ Temp: <strong>${temp}°C</strong> (feels like ${feels}°C)<br />☁️ ${desc}<br />💨 Wind: ${wind} km/h | 💧 Humidity: ${humid}%<br /><br /><strong>⚡ POWERED BY ICEBACK MASTER TECH</strong>`
                };
            } catch (e) {
                return {
                    reply:
        ...
        // 4. SAFARI SEARCH (safari search London, safari search Eiffel Tower)
    ...
    // ===== AI CHAT SEND (FIXED - ALWAYS REPLIES) =====
    async function sendChatMessage() {
        ...
        try {
            // STEP 1: Check for direct commands (weather, search, owner)
            const directReply = await handleDirectCommand(text);
            if (directReply) {
            ...
            // STEP 2: Try AI APIs with timeout
            ...
            // STEP 3: If both APIs failed, use the smart fallback
            if (!apiSuccess || !reply) {
    ...
    async function generateImage() {
        ...
        const url =
            `${CONFIG.IMAGE_API}/${encodeURIComponent(fullPrompt)}?width=512&height=512&nologo=true&seed=${Date.now()}`;
